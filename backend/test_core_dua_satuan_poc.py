...
def cleanup(db, state: Dict[str, List[str]], audit_before: set,
            stock_snap: Any, stock_before: Dict[str, int]) -> None:
    ...
    removed += db.notifications.delete_many({"message": {"$regex": TAG}}).deleted_count
...
def main() -> int:
    ...
    try:
        ...
        cleanup(db, state, audit_before, stock_snap, stock_before)
